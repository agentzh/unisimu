...
sub dump_result {
    my ($cgi, $x, $y) = @_;
    my $newton = Interp::Newton::Diffs->new(Xs => $x, Ys => $y);
    my @cols = $newton->diffs;
    my $poly = $newton->raw_poly;
    my $poly2 = $newton->polynomial;
    my $poly3 = $newton->maple->eval("collect($poly2,x)");
    ...
    $tt->process(
        ...
        { step => 1, data => [ $x, @cols ], Xs => $x, Ys => $y,
          poly => $poly, simple_poly => $poly3, test_res => $res},
        \*STDOUT,
    ) || warn $tt->error();
}
